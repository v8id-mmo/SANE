...
value, including right at the `-128`/`127` boundary: equality only cares
whether two bit patterns match, not how the sign bit is interpreted, so
it isn't affected by the boundary-value risk that applies to signed byte
