...
the `long` value. This is the same underlying gap as `bankbyte`'s
known limitation on `long`. If you need a byte out of a `long` value,
read it directly instead (e.g. via an overlapping `array[3] of byte` at
the same address) rather than through `Hi`/`Lo`/`bankbyte`.
